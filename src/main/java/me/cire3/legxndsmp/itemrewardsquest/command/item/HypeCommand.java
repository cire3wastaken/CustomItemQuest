...
public class HypeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String label, String[] strings) {
        ...
        if(commandSender.hasPermission("itemrewardsquest.giveitems")  || commandSender.isOp()){
            ...
            meta.setLore(ColorUtils.color(ItemRewardsQuest.INSTANCE.hyperion.lore));
            item.setItemMeta(meta);

            Player target = Bukkit.getPlayerExact(strings[0]);
            if (target == null) {
                commandSender.sendMessage(FAIL_PREFIX + strings[0] + " is not online!");
                return true;
            }

            target.getInventory().addItem(item);
            commandSender.sendMessage(CHAT_PREFIX + "Successfully gave " +
